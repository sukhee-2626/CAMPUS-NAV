...
var recordedCampusPoints = mysqlTable("recorded_campus_points", {
  id: int("id").autoincrement().primaryKey(),
...
async function listRecordedCampusPoints(userId) {
  const db = await getDb();
  if (!db) return [];
...
async function createRecordedCampusPoint(point) {
  const db = await getDb();
...
async function deleteRecordedCampusPoint(userId, pointId) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
...
var appRouter = router({
  ...
  recordedPoints: router({
    ...
    create: protectedProcedure.input(z2.object({
      ...
      category: z2.string().min(1).max(80).default("campus-point"),
      latitude: z2.number().gte(-90).lte(90),
      longitude: z2.number().gte(-180).lte(180),
      accuracyMeters: z2.number().int().nonnegative().optional(),
      notes: z2.string().max(1e3).optional()
    })).mutation(({ ctx, input }) => createRecordedCampusPoint({ ...input, userId: ctx.user.id })),
    delete: protectedProcedure.input(z2.object({ id: z2.number().int().positive() })).mutation(({ ctx, input }) => deleteRecordedCampusPoint(ctx.user.id, input.id))
  })
